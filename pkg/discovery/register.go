...
func NewRegister(ctx context.Context, etcdAddrs []string) *Register {
	return &Register{
		EtcdAddrs:   etcdAddrs,
		DialTimeout: 3,
		ctx:         ctx,
...
func (r *Register) Stop() {
	...
	if err := r.unregister(); err != nil {
		log.ZError(r.ctx, "unregister failed, error: ", err)
	}

	if _, err := r.cli.Revoke(context.Background(), r.leasesID); err != nil {
		log.ZError(r.ctx, "revoke failed, error: ", err)
	}
}
...
func (r *Register) keepAlive() {
	...
	for {
		...
		case res := <-r.keepAliveCh:
			if res == nil {
				if err := r.register(); err != nil {
					log.ZError(r.ctx, "register failed, error: ", err)
				}
			}
		case <-ticker.C:
